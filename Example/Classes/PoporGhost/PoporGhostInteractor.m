...
@implementation PoporGhostInteractor
...
- (void)fetchArray:(NSString *)className {
    ...
    basePath = [basePath stringByAppendingPathComponent:@"YYCache_PoporGhost"];
    ...
    self.yyDiskCache.customUnarchiveBlock = ^id _Nonnull(NSData * _Nonnull data) {
        ...
        for (id oneID in array) {
            [mArray addObject:[PoporGhostListEntity yy_modelWithDictionary:oneID]];
        }
        return mArray;
    };
...
- (void)addDic:(NSDictionary *)dic des:(NSString *)description {
    ...
    {
        PoporGhostListEntity * entity = [PoporGhostListEntity new];
